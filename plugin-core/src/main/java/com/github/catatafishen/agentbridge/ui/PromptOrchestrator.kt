...
class PromptOrchestrator(
    ...
    fun execute(
        ...
        try {
            executePrompt(prompt, contextItems, selectedModelId, isAutoCommit, isContinue)
        } finally {
        ...
        if (sessionId != lastInitialisedSessionId) {
            ...
            if (!savedModel.isNullOrEmpty()) {
                val availableModels = try {
                    ...
                    log.warn("Failed to retrieve available models for validation — skipping setModel", ex)
                    emptyList()
                }
                if (availableModels.isEmpty() || availableModels.any { it.id() == savedModel }) {
                    ...
                    log.warn("Saved model '$savedModel' not in available models — skipping setModel to avoid empty turn")
                }
            }
    ...
    private fun prepareModelAndTurnState(
        selectedModelId: String,
        isAutoCommit: Boolean = false,
        isContinue: Boolean = false
    ): String {
        ...
        if (!isAutoCommit && !isContinue) LiveToolCallService.getInstance(project).clear()
    ...
    private fun buildEffectivePrompt(prompt: String, attachments: List<PromptAttachment>): String {
        ...
        if (ActiveAgentManager.getInjectConversationHistory(project)) {
            ...
            if (summary.isNotEmpty()) {
                ...
                effective = if (effective.trimStart().startsWith("/")) {
                    "$effective\n\n$summary"
                } else {
                    "$summary\n\n$effective"
                }
        ...
        effective = appendAutoCommitInstruction(effective)
        return effective
    }

    /**
     * Appends a clear instruction block telling the agent whether it should
     * auto-commit changes or leave them for the user. Reads the live setting
     * from [McpServerSettings.isAutoCommit] so the instruction always matches
     * the current toggle regardless of who triggered the turn.
     *
     * Respects Git Policy: SAFE mode blocks git write tools entirely.
     */
    private fun appendAutoCommitInstruction(prompt: String): String {
        ...
        val autoCommitOn = settings.isAutoCommit
    ...
    private fun handleSessionCorrupted() {
        ...
        val displayMsg = PromptErrorClassifier.classifyEmptyTurn(
            agentName = agentName,
            pendingBannerMessage = pendingBanner?.message,
