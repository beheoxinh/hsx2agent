...
class ChatToolWindowContent(
    ...
    private val rootSplitter = OnePixelSplitter(false, 0.3f).apply {
        setHonorComponentsMinimumSize(false)
        addPropertyChangeListener("proportion") { evt ->
            if (isVisible && sidePanel?.isVisible == true) {
                val p = evt.newValue as Float
                val position = ChatInputSettings.getInstance().sidePanelPosition
                val isClosed = if (position == SidePanelPosition.RIGHT || position == SidePanelPosition.BOTTOM) {
                    p >= 0.99f
                } else {
                    p <= 0.01f
                }
    ...
    private fun restoreSidePanelOpenState() {
        ...
        if (props.getBoolean(PREF_SIDE_PANEL_OPEN, true)) {
            ensureSidePanelAvailable()
            sidePanel?.isVisible = true
            val position = ChatInputSettings.getInstance().sidePanelPosition
            val p = getSavedProportion()
            rootSplitter.proportion = if (position == SidePanelPosition.RIGHT || position == SidePanelPosition.BOTTOM) {
                1.0f - p
            } else {
                p
            }
    ...
    private fun installInputResizeHandler(
        ...
        // N handler — attached to inputSection.
    ...
    private fun saveProportion(proportion: Float) {
        val position = ChatInputSettings.getInstance().sidePanelPosition
        val p = if (position == SidePanelPosition.RIGHT || position == SidePanelPosition.BOTTOM) {
            1.0f - proportion
        } else {
            proportion
        }
        com.intellij.ide.util.PropertiesComponent.getInstance(project)
            .setValue(PREF_SIDE_PANEL_PROPORTION, p, 0.3f)
    }

    private fun getSavedProportion(): Float {
        return com.intellij.ide.util.PropertiesComponent.getInstance(project)
            .getFloat(PREF_SIDE_PANEL_PROPORTION, 0.3f)
    }

    fun updateSidePanelLayout() {
        val position = ChatInputSettings.getInstance().sidePanelPosition
        val side = if (sidePanel?.isVisible == true) sidePanel else null
        val main = mainPanel

        rootSplitter.orientation = position.isVertical()

        // Clear components first to avoid Swing re-parenting issues in Splitter.
        // If we swap first/second and they are the same component, Splitter's internal
        // remove() call for the old position can accidentally remove the newly-added
        // component from the container.
        rootSplitter.firstComponent = null
        rootSplitter.secondComponent = null

        if (position == SidePanelPosition.LEFT || position == SidePanelPosition.TOP) {
            rootSplitter.firstComponent = side
            rootSplitter.secondComponent = main
            rootSplitter.dividerPositionStrategy =
                com.intellij.openapi.ui.Splitter.DividerPositionStrategy.KEEP_SECOND_SIZE
        } else {
            rootSplitter.firstComponent = main
            rootSplitter.secondComponent = side
            rootSplitter.dividerPositionStrategy =
                com.intellij.openapi.ui.Splitter.DividerPositionStrategy.KEEP_FIRST_SIZE
        }
        ...
        if (side == null) {
            ...
            val p = getSavedProportion()
            val target = if (position == SidePanelPosition.RIGHT || position == SidePanelPosition.BOTTOM) {
                1.0f - p
            } else {
                p
            }
            // Only update if significantly different to avoid feedback loops
            if (kotlin.math.abs(rootSplitter.proportion - target) > 0.001f) {
                rootSplitter.proportion = target
            }
            // Force refresh tabs if we are in multi-tab mode to ensure native IntelliJ tabs
            // in the tool window header are re-rendered correctly for the new position.
            sidePanel?.updateLayoutOrientation(position)
            if (contentWrappers.isNotEmpty()) {
                updateSideTabContents(true)
            }
        }

        rootSplitter.revalidate()
        rootSplitter.repaint()
    }
    ...
    private inner class SidePanelToggleAction : ToggleAction(
        ...
        override fun isSelected(e: AnActionEvent): Boolean {
            ...
            return if (position == SidePanelPosition.RIGHT || position == SidePanelPosition.BOTTOM) {
                p < 0.99f
            } else {
                p > 0.01f
            }
    ...
    private fun notifyIfUnfocused(toolCallCount: Int) {
        ...
        ApplicationManager.getApplication().invokeLater {
            ...
            com.intellij.openapi.wm.ToolWindowManager.getInstance(project)
                .notifyByBalloon(
                    com.github.catatafishen.agentbridge.ui.ChatToolWindowFactory.ID,
