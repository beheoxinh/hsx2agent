...
| `pluginTool`   | `String?` | `null`       | Yes     |  ✅ (`pluginTool`)  |  ✅ (`pluginTool`)  |
| `timestamp`    | `String`  | `""`         | No      |      ✅ (`ts`)      |  ✅ (`timestamp`)   |
| `agent`        | `String`  | `""`         | No      |    ✅ (`agent`)     |    ✅ (`agent`)     |
| `model`        | `String`  | `""`         | No      |         ❌          |    ✅ (`model`)     |
| `entryId`      | `String`  | UUID         | No      |     ✅ (`eid`)      |   ✅ (`entryId`)    |
...
- DOM message limit: sent via `/state` response (`domMessageLimit` field) and via `ChatController.setDomMessageLimit()`
  JS call
